  ...
  async checkTasksForNotifications() {
    try {
      ...
      for (const task of tasks) {
        ...
        if (task.start_time && task.due_time && !this.notificationsSent.start.has(task.id)) {
        ...
        if (task.end_time && task.due_time && !this.notificationsSent.end.has(task.id)) {
          const endTime = new Date(`${task.due_time}T${task.end_time}`);
